...
const SidebarMenu = () => {
  ...
  const filterMenuByLevels = (menuConfig: MenuConfigType, level: string): MenuConfigType => {
    return menuConfig
      .map(menu => {
        // Lọc các menu con dựa trên quyền
        const filteredChildren = menu.children?.filter(child => {  
          // Kiểm tra quyền cho mỗi menu con
          if (level == ('Root')) {
            return true;
          }
        ...
        // Nếu tất cả các mục con đều bị ẩn, ẩn luôn cả menu cha
        if (!filteredChildren?.length) {
          return null; // Nếu không có mục con nào hợp lệ, trả về null để loại bỏ menu cha
        }
  
        // Trả về menu với các mục con đã lọc
        return { ...menu, children: filteredChildren };
      })
      .filter(menu => menu !== null); // Loại bỏ menu cha nếu không có mục con hợp lệ
  };

  const filteredMenuConfig = filterMenuByLevels(menuConfig || [], level);
